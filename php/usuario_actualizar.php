<?php
    ...
    $check_admin=$check_admin->query("SELECT usuario, clave FROM usuarios WHERE usuario='$admin_usuario' AND usuario_id='".$_SESSION['usuario_id']."'");
    ...
    if($usuario!=$datos['usuario']){
	    $check_usuario=conexion();
	    $check_usuario=$check_usuario->query("SELECT usuario FROM usuarios WHERE uusuario='$usuario'");
	    if($check_usuario->rowCount()>0){
	        echo '
	            <div class="notification is-danger is-light">
	                <strong>¡Ocurrio un error inesperado!</strong><br>
    ...
    $actualizar_usuario=$actualizar_usuario->prepare("UPDATE usuarios SET nombre=:nombre,apellido=:apellido,usuario=:usuario,clave=:clave,email=:email WHERE usuario_id=:id");
